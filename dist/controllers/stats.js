...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const latestTransactionPromise = Order.find({})
            ...
            .limit(5);
        const [prodThisMonth, prodLastMonth, userThisMonth, userLastMonth, orderThisMonth, orderLastMonth, lastSixMonthOrders, latestTransaction, productCount, userCount, allOrders, category, femaleUserCount,] = await Promise.all([
...
export const getPieChart = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [processingOrder, shippedOrder, deliveredOrder, category, productCount, outOfStock, allOrders, allUsers, adminUsers, customerUsers,] = await Promise.all([
